...
`scripts/download_from_huggingface.py`는 Hugging Face PubLayNet을 내려받아 다음
COCO layout으로 변환하는 독립 데이터 준비 명령이다.
...
DINO의 `encoder_enabled`는 deformable encoder layer만 ablation한다. `true`이면
`enc_layers` 값을 사용하고 `false`이면 vendored DINO에 `enc_layers=0`을 전달한다.
이때 encoder memory는 입력 P2~P5 token과 같지만 `two_stage_type=standard` 기반
proposal 선택, deformable decoder, denoising과 matching은 유지된다. ON/OFF 실험은
parameter 구성이 다르므로 서로 다른 `output_dir`과 `weights_dir`을 사용한다.
...
보이는 CUDA device 기준이다. 부모는 각 child에 rank, world size, 선택 GPU와
TCP init URL을 인자로 직접 전달하며 child가 NCCL process group을 생성하고 해제한다.
외부 launcher 환경변수는 읽지 않는다. 두 backend 모두 `training.batch_size`는 global batch이고
...
`tracking.enabled: true`가 기본이다. run마다 detector/action tag, fresh 및 resume별
effective config, train loss/LR, evaluation metric을 기록한다. DINO는 학습 callback,
Cascade는 평가 주기와 학습 log 주기를 분리한 Detectron2 hook을 사용한다. 정상 종료된
run은 resume 시 같은 history를 이어 쓰고, 실패·중단된 run은 `resume_of`로 연결된 새
attempt에서 시작해 동일 step 중복을 막는다. 로컬 기본 저장소는
`sqlite:///mlflow.db`이며 원격 server URI도 YAML로 변경할 수 있다. 끄려면
`--options tracking.enabled=false`를 사용한다.
