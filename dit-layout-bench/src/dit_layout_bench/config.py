"""Shared TOML configuration, CLI overrides and runtime validation."""
...
def load_settings(
    ...
    """Load defaults, merge an optional partial TOML file, then CLI overrides."""
    ...
    if selected.resolve() != DEFAULT_CONFIG.resolve():
        _merge(settings, tomllib.loads(selected.read_text(encoding="utf-8")))
