...
class SourceTests(unittest.TestCase):
    ...
    def test_train_owns_ddp_lifecycle_and_cascade_restores_nested_state(self):
        entrypoint = (ROOT / "train.py").read_text(encoding="utf-8")
        backend = (
            ROOT / "src" / "dit_layout_bench" / "backends" / "cascade_rcnn.py"
        ).read_text(encoding="utf-8")
        self.assertIn("with distributed_session(", entrypoint)
        self.assertNotIn("distributed_session", backend)
        self.assertIn("api.comm.create_local_process_group", backend)
        self.assertIn('checkpoint.get("resume_iteration", trainer.iter + 1)', backend)
