...
# ------------------------------------------------------------------------
# Modified: eval 시 coco_instances_results.json 저장 추가 (baseline_dit_cascade의 detectron2
# COCOEvaluator 출력과 동일 포맷으로 맞춰 common/coco_perclass_eval.py가 양쪽을 동일하게 처리).
# ------------------------------------------------------------------------
import argparse
import datetime
import json
    ...
    if args.eval:
        ...
        if args.output_dir:
            ...
            if coco_evaluator is not None and "bbox" in coco_evaluator.coco_eval:
                coco_evaluator.dump(str(output_dir / "coco_instances_results.json"))
