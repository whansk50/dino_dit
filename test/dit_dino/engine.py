...
# Modified: train_one_epoch 반환값에 max_mem(누적 최대 CUDA 메모리, MB) 추가 —
# detectron2 metrics.json의 max_mem과 동일한 정의로 baseline과 대칭을 맞춘다.
"""
...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    if torch.cuda.is_available():
        # detectron2의 max_mem(누적 최대, 리셋 없음)과 동일한 정의로 맞춘다
        resstat['max_mem'] = torch.cuda.max_memory_allocated() / 1024.0 / 1024.0
    return resstat
