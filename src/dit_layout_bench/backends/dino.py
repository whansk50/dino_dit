...
def _training_epoch_metrics(stats: dict[str, Any]) -> dict[str, Any]:
    """Give raw and weighted DINO epoch losses unambiguous metric names."""
    metrics = {}
    for name, value in stats.items():
        if name.endswith("_unscaled"):
            metrics[name.removesuffix("_unscaled")] = value
        elif f"{name}_unscaled" in stats and name != "class_error":
            metrics[f"{name}_scaled"] = value
        else:
            metrics[name] = value
    return metrics
...
def _effective_dino_values(
    ...
    return {
        ...
        # Zero encoder layers makes the vendored TransformerEncoder an
        # identity while preserving two-stage proposal selection, the
        # deformable decoder, denoising, and matching. This isolates the
        # encoder contribution for the DiT+DINO ablation.
        "enc_layers": (
            detector["enc_layers"] if detector["encoder_enabled"] else 0
        ),
...
def _build_dino_integration(config: RunConfig) -> DinoIntegration:
    ...
    def train_epoch_callback(stats, epoch):
        tracker = active_tracker()
        if tracker is None:
            return
        tracker.log_metrics(
            {
                ...
                for key, value in _training_epoch_metrics(stats).items()
            },
            step=epoch,
        )
    ...
    return DinoIntegration(
        ...
        on_train_epoch=train_epoch_callback,
    )
