...
Hugging Face PubLayNet을 다음 명령으로 내려받아 COCO layout으로 변환할 수 있다.

```bash
python scripts/download_from_huggingface.py --output-dir publaynet
```

이미 준비한 PubLayNet을 사용하는 경우에도 다음 layout을 따라야 한다.
...
### DINO encoder ablation

`dino.encoder_enabled`로 DINO의 deformable encoder layer를 켜거나 끌 수 있다.
기본값은 `true`이며 `dino.enc_layers`에 지정한 layer 수를 사용한다. `false`이면
런타임 layer 수를 0으로 전달해 encoder를 identity로 만든다.

```bash
python train.py \
  --config configs/dino_train.yaml \
  --output-dir outputs/dino-encoder-off \
  --weights-dir outputs/dino-encoder-off/weights \
  --options dino.encoder_enabled=false
```

이 모드는 `two_stage_type=standard`를 유지하므로 raw P2~P5 memory에서 수행하는
top-k proposal 선택, deformable decoder, denoising과 Hungarian matching은 그대로
동작한다. 따라서 일반적인 decoder-only detector가 아니라 DINO encoder layer의
기여만 비교하는 ablation이다. ON/OFF 모델은 parameter 구성이 달라 checkpoint를
서로 resume할 수 없으므로 별도 output/weights 디렉터리를 사용해야 한다.
...
부모는 `torch.multiprocessing.spawn`으로 process를 만들고 각 process에 rank,
world size와 선택 GPU를 직접 전달한다. 각 child는 전달받은 값으로
`torch.distributed` NCCL process group을 생성하고 종료 시 해제한다.
...
- fresh run의 `effective-config.yaml`과 resume별 `resume-configs/` 설정
- 학습 run의 global/per-GPU batch와 모든 run의 world size
- train loss/LR 및 COCO evaluation metric
- 추론 이미지와 prediction 수

정상 종료된 동일 detector run을 resume하면 `weights/mlflow-run-id.txt`의 run ID를
재사용해 history를 이어 쓴다. 이전 attempt가 `FAILED`, `KILLED`, `RUNNING` 상태이거나
tracking store에서 사라졌다면 중복 step을 만들지 않도록 `resume_of` tag가 붙은 새
attempt를 시작한다. DINO는 raw loss와 `_scaled` loss 및 `train_epoch/*`를 구분하고,
Cascade는 학습 주기와 무관하게 평가가 발생한 step의 `eval/*` metric을 기록한다.
...
│   ├── download_from_huggingface.py  # PubLayNet 다운로드 및 COCO 변환
