...
def _verify_checkpoint(path: Path, *, expected_iteration: int) -> None:
    ...
    if int(checkpoint.get("resume_iteration", -1)) != expected_iteration:
        raise RuntimeError(
            f"Expected resume iteration {expected_iteration}, "
            f"got {checkpoint.get('resume_iteration')}"
        )
...
def _verify_mlflow(work_dir: Path, *, world_size: int) -> None:
    ...
    for run in runs:
        ...
        required_metrics = {"train/total_loss", "eval/bbox/AP"}
        missing = required_metrics.difference(run.data.metrics)
        if missing:
            raise RuntimeError(f"MLflow metrics are missing: {sorted(missing)}")
        if any(key.startswith("train/bbox/") for key in run.data.metrics):
            raise RuntimeError("Evaluation metrics were logged under train/")
        for key in ("train/total_loss", "eval/bbox/AP"):
...
def main(argv: list[str] | None = None) -> None:
    ...
    try:
        ...
        _verify_checkpoint(
            checkpoint, expected_iteration=iterations_per_epoch
        )
        ...
        _verify_checkpoint(
            checkpoint, expected_iteration=iterations_per_epoch * 2
        )
